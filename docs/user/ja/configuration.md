...
VS Code 上でこの設定画面へ直接移動したい場合は、コマンドパレットから **CustomViewer: 設定を開く** を実行してください。
...
制限モードは「まず開いて確認し、信頼はあとで決める」ための機能だと考えると分かりやすいです。初見のリポジトリを開くときに、そのワークスペースが持つプレビューコードをすぐには動かさないようにできます。
...
自分が普段使うプロジェクトで、同梱レンダラーも使いたいならワークスペースを信頼してください。逆に、他人のリポジトリを安全側で確認したいだけなら、制限モードのままにしておくのが向いています。
